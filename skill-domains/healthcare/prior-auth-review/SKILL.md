...
### Step 11 — Present findings as structured text

Do NOT generate HTML or a dashboard. Output a clean structured text summary using the data collected in Steps 1–10.

Use this format:

---

**PA REVIEW SUMMARY — {auth_request_num}**
Generated: {today's date}

**AUTHORIZATION HEADER**
| Field | Value |
|-------|-------|
| PA Number | {auth_request_num} |
| Auth Status | {auth_status} |
| Member | {first_name} {last_name} · {member_id_num} · {gender} |
| Provider | Dr. {provider_name} · {primary_specialty} · NPI {npi} |
| Date of Service | {requested_dos} |

**METRICS**
- CPT Codes Requested: {count}
- Prerequisites Required: {prereq_count}
- Prerequisites Satisfied: {satisfied}/{prereq_count}
- Prior Claims Found: {claims_count}

**ICD-10 DIAGNOSES**
List each diagnosis code and description (primary first, secondary after).

**CARE PATHWAY GAPS**
For each CPT code that has prerequisites, list:

| Prerequisite CPT | Description | Required Condition | Status |
|-----------------|-------------|-------------------|--------|
| {cpt_prereq} | {desc} | {requirement} | 🔴 Missing / 🟡 Partial / 🟢 Satisfied |

If a CPT code has no prerequisites, list it under **Auto-Approved**.

**CLAIMS HISTORY**
Summarize what was found in WORKERS_COMP_HDR/DTL for the prerequisite CPT codes prior to {requested_dos}.
If empty: "No prior claims found for {member_id_num} (searched for {prereq_codes} before {requested_dos})."

**CLINICAL NOTES**
List up to 3 most recent notes:
- {note_type} · {note_date}: {first 150 chars of note_text}…
- If prior_test_name is not null: "Prior Test: {prior_test_name} — {prior_test_result}"

**AUTO-APPROVED CPT CODES**
List CPT codes with no prerequisites as: ✓ {cpt_code} — {description} (no action required)

**ACTION REQUIRED** *(only if gaps exist)*
For each RED or AMBER prerequisite:
→ Submit {prereq_cpt} ({prereq_desc}) records: dates, provider notes, duration, outcomes

If auth status is not approved, applicable denial codes:
- `44` — Documentation of conservative treatment failure required
...
> To generate the full care pathway dashboard with visual charts and tables, type:
> **"Generate the PA dashboard for {auth_request_num}"**
